      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between">
            ...
            <div className={`w-3 h-3 rounded-full ${isSchedulerActive ? 'bg-green-500' : 'bg-gray-400'}`}></div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-600">アクティブ設定</p>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-green-600">{publishedToday.length}</p>
            </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        <div className="flex items-center space-x-3">
                          ...
                          <span className={`px-2 py-1 rounded-full text-xs ${
                            config.isActive ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-600'
                          }`}>
                            {config.isActive ? 'アクティブ' : '非アクティブ'}
                          </span>
                          {config.scheduleSettings?.isActive && (
                        ...
                        <div className="mt-2 space-y-1">
                          <div className="flex items-center space-x-4 text-sm text-gray-600">
                            ...
                            <span>カテゴリ: {config.category || config.defaultCategory || '未設定'}</span>
                          </div>
                          
                          {config.scheduleSettings && (
                            <div className="flex items-center space-x-4 text-sm text-gray-600">
                              <span>頻度: {config.scheduleSettings.frequency === 'daily' ? '毎日' :
                                config.scheduleSettings.frequency === 'weekly' ? '毎週' :
                                config.scheduleSettings.frequency === 'biweekly' ? '隔週' : '毎月'}</span>
                              <span>時刻: {config.scheduleSettings.time}</span>
                              <span className={getPublishStatusColor(config.scheduleSettings.publishStatus)}>
                                投稿状態: {getPublishStatusText(config.scheduleSettings.publishStatus)}
                              </span>
                            </div>
                          )}
                          
                          {config.scheduleSettings?.targetKeywords && (
                            <div className="text-sm text-gray-600">
                              <span>キーワード: {totalKeywordsCount}個設定済み</span>
                              {usedKeywordsCount > 0 && (
                                <span className="ml-2 text-orange-600">
                                  ({usedKeywordsCount}個使用済み)
                                </span>
                              )}
                            </div>
                          )}
                          ...
                          {config.scheduleSettings?.isActive && (
                            <div className="text-sm text-gray-600">
                              次回実行予定: {formatNextExecutionTime(config.id, config.scheduleSettings)}
                            </div>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="flex items-center space-x-3">
                        <button
                          onClick={() => toggleConfigSchedule(config.id)}
                          disabled={!config.scheduleSettings?.targetKeywords?.length}
                        ...
                        >
                          {config.scheduleSettings?.isActive ? '停止' : '開始'}
                        </button>
                      </div>
                    </div>
                  </div>
          ...
          <div className="space-y-4">
            ...
            <div className="border-t border-gray-200 pt-4">
              <button
                onClick={async () => {
                  try {
                    toast.loading('サーバーサイドスケジューラーを実行中...', { duration: 3000 });
                    const { supabaseSchedulerService } = await import('../services/supabaseSchedulerService');
                    const result = await supabaseSchedulerService.triggerScheduler();
                    console.log('Scheduler result:', result);
                    toast.success(`スケジューラー実行完了: ${result.executed || 0}件の記事を処理しました`);
                  } catch (error: any) {
                    console.error('Scheduler trigger error:', error);
                    toast.error(`スケジューラー実行エラー: ${error.message}`);
                  }
                }}
                className="w-full btn-primary flex items-center justify-center space-x-2"
              >
                <Zap className="w-4 h-4" />
                <span>サーバーサイドスケジューラーを今すぐ実行</span>
              </button>
              <p className="text-xs text-gray-500 mt-2">
                ※ 設定された時刻条件に関わらず、すぐにスケジュール処理を実行します
              </p>
            </div>
          </div>
        </div>
